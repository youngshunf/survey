   ...
   /**
     ...
     */
     public function actionChooseTemplate($id){
        ...
        $dataProvider=new ActiveDataProvider([
            'query'=>Template::find()->andWhere(['user_guid'=>$user_guid])->orderBy('created_at desc'),
        ]);
 ...
 public function actionUpdateQuestionDo(){
    ...
    public function actionFirstAuth(){
        ...
        if($answer->save()){
            ...
            $message->sendMessage(null,$answer->user_guid,$title, $content,2);
...
//导出问题答案
    public function actionExportAnswer($id,$status){
        ...
        foreach ($answerArr as $key=>$item){
            ...
            $resultExcel->getActiveSheet()->setCellValue('E'.$i,empty($item['user'])?'':CommonUtil::truncateMobile($item['user']['mobile']));
