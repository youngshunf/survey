...
class ImageUploader{
     ...
     * @param string $name
     */
    public static function uploadByName($name,$str=null){
        ...
        if(empty($photoFile)){
            ...
            $waterMask=new WaterMask($basePath.$path.$fileName);
            $waterMask->waterType=0;
            $waterMask->pos=8;
            if($str!=NULL){
                $waterMask->waterStr=$str;
            }
            $waterMask->fontSize=16;
            $waterMask->fontFile=__DIR__.'/yahei.ttf';
            $waterMask->output();
     ...
     * @param string $name
     */
    public static function uploadHeadImgByName($name,$str=NULL){
        $photoFile=UploadedFile::getInstanceByName($name);
        if(empty($photoFile)){
            return false;
        }
   ...
   public  static function  uploadImageByBase64($imgData,$imgLen,$str,$answerTime=null){  
       ...
       if(is_file($basePath.$path.$fileName)){
           ...
           $waterMask=new WaterMask($filePath.$fileName);
           $waterMask->waterType=0;
           $waterMask->waterStr=$str;
           $waterMask->pos=9;
           $waterMask->fontSize=18;
           $waterMask->fontFile=__DIR__.'/yahei.ttf';
           $waterMask->output(); 
           ...
           $waterMask->waterType=0;
           if(!empty($answerTime)){
               $waterMask->waterStr=date('Y-m-d H:i:s',$answerTime);
           }else{
               $waterMask->waterStr=date('Y-m-d H:i:s');
           }
           $waterMask->pos=10;
           $waterMask->fontSize=18;
           $waterMask->fontFile=__DIR__.'/yahei.ttf';
   ...
   public  static function  uploadHeadImageByBase64($imgData,$imgLen){
       if(strlen($imgData)!=$imgLen){
           return false;
       }
       $basePath='../../upload/avatar/';
