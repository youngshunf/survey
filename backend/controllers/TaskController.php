...
class TaskController extends Controller
{
    ...
    public function actionCopyTask($id){
    ...
    public function actionOnLine(){
        ...
        Task::updateAll(['status'=>1,'end_time'=>$end_time],['id'=>$id]);
    ...
    public function actionChooseTemplateDo(){
        ...
        if($project_id){
            ...
            yii::$app->getSession()->setFlash('success','模板应用成功!');
...
/*     public function actionViewAnswerDetail($id){
    ...
    public function actionDeleteQuestion($id){
        ...
        if(!empty($question->project_id)){
            Question::deleteAll(['project_id'=>$question->project_id,'code'=>$question->code]);
        }else{
            $question->delete();
        }
            ...
            if(!empty($downquestion)){
                ...
                if($downquestion->save()){
                    ...
                    if($question->project_id){
                        Question::updateAll(['project_id'=>$question->project_id,['code'=>$question->code]],['code'=>$downcode]);
                    }
                    if($question->save()){
                        yii::$app->getSession()->setFlash('success','问题下移成功');
                        return $this->redirect(yii::$app->request->referrer);
                    }
                }
//                 if($question->project_id){
//                     $downquestionArr=Question::findAll(['task_guid'=>$question->task_guid,'code'=>$downcode]);
//                     Question::updateAll(['project_id'=>$question->project_id,['code'=>$downcode]],['code'=>$question->code]);
//                 }
            }
         }
    ...
    public function actionPublishTaskTemplate(){
        ...
        try{
            ...
            $taskTemplate->is_show_price=$task->is_show_price;
