				...
				<?php }else{?>
        				...
        				<li class="mui-table-view-cell">
        				   <p style="margin-top: 8px;">
        						    	...
        						    	<?php
                                        if($answer->status==1 ||$answer->status==99){//任务已提交不能再提交
        						    	if($taskArr['dist']*30<=$taskArr['do_radius']){?>
        						    	<a class="btn  btn-warning pull-right do-task"  task_guid="<?= $taskArr['task_guid']?>"> 做任务</a>
        					           <?php }else{?>
        					           <span class="btn btn-default pull-right">不在执行范围</span>
        					           <?php } }?>
        					 </p>
        				</li>
        				<?php }}else{?>
        				<li class="mui-table-view-cell">
        					 <p class="center"><a class="btn btn-danger get-task"  href="#"   task_guid=<?= $taskArr['task_guid']?>> 领取任务</a>					 					 	
        					 </p>
        				</li>
        				<?php }?>
